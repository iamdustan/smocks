...
- added ability to use custom `respondWithFile` handler
- added `initialState` smocks option which will be used as the server state when started (or reset)
- fixed `boolean` input rendering on admin panel
- added `route` and `variant` as `this` attributes in route response handlers
